"""A-15 · Weather Variable Pairplot — multivariate co-factor matrix."""
...
from utils.formatters import hover_correlation, hover_pairplot_scatter, hover_template
...
def _correlation_fallback(data, dashboard, cfg):
    ...
    fig = go.Figure(
        ...
        go.Heatmap(
            ...
            hovertemplate=hover_template("%{y} × %{x}", hover_correlation()),
    ...
    return apply_dashboard_theme(
        ...
        fig, dashboard, role=cfg.get("role", "hero"), show_legend=False, chart_type="pairplot"
...
def render(data, config=None):
    ...
    visible_categories = cfg.get("visible_categories") or []

    plot_data = data

    if visible_categories and COL_AQI_CATEGORY in data.columns:

        plot_data = data[data[COL_AQI_CATEGORY].isin(visible_categories)]

        if plot_data.empty:

            return empty_figure("No rows for selected PM2.5 categories", dashboard)



    sample = plot_data.sample(min(len(plot_data), 600), random_state=42) if len(plot_data) > 600 else plot_data

    n = len(vars_)

    labels = [v.replace("_", " ") for v in vars_]

    spacing = 0.09 if n <= 5 else 0.07
                    ...
                    for cat in sample[COL_AQI_CATEGORY].dropna().unique():
                        ...
                        fig.add_trace(
                            ...
                            go.Scatter(
                                ...
                                hovertemplate=hover_template(hover_pairplot_scatter()),
