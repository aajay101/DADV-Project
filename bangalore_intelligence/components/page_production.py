...
def render_production_page(
    ...
    with st.expander("ACTIVE FILTER CONTEXT", expanded=False):
        st.caption(f"Dashboard: {dashboard}")
        st.caption(f"Page: {page_key}")
        st.caption(f"Rows after global filters: {bundle.get('record_count', 'n/a')}")
        st.caption(f"Active filters: {filter_snapshot}")
        st.caption("Analytical Workspace inherits global filters; local controls only narrow displayed overlays.")
    ...
    if dashboard == "traffic" and bundle.get("kpi_methodology"):
        from data_layer.traffic_transforms import get_traffic_kpi_methodology_catalog

        with st.expander("KPI definitions (filtered data)", expanded=False):
            for item in get_traffic_kpi_methodology_catalog():
                st.caption(f"**{item['label']}** — {item['definition']}")
    command_zone_close()
