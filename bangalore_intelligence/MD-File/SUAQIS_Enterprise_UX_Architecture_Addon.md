...
```
Format: "n = 1,247 records"
Position: bottom-right corner of chart_container
Font: 10px, TEXT_MUTED, opacity 0.8
Color: TEXT_MUTED (neutral always — even at n=5)
```

**Low-sample warning trigger (n < 200):**
...
```
"⚠ Insufficient data for visualization (n = [count]).
...
"You've been reviewing this dashboard for 90+ minutes. Consider exporting a report summary before continuing."
[Export Summary]  [Dismiss]
```
...
# PART 7 — EXPORT / REPORTING UX ADDON

---

## 7.1 · Export Architecture Philosophy

Analysts regularly need to share dashboard insights in external formats — executive briefings, environmental compliance reports, operational handover documents. The export system must make this workflow fast and produce outputs that are self-contained: a recipient who has never seen the dashboard should be able to interpret an exported chart or PDF report without needing to return to the platform.

**Self-contained export rule:** Every export includes: the chart/KPI content, the active filter state that produced it, the dataset date range, the platform name and timestamp, and any active annotations. An exported PNG of T-05 Quadrant Scatter should show the quadrant labels, the reference lines, the axis labels, and a footer stating "Traffic Intelligence · Bangalore Urban Intelligence Platform · Filters: Koramangala, Jan–Jun 2023 · Generated: 2024-08-15 14:32."

---

## 7.2 · Per-Chart PNG Export

Every chart container includes a `⬇ PNG` export control accessible via the fullscreen-mode toolbar (not visible by default to reduce visual noise in the primary view).

**PNG export behavior:**

```python
def export_chart_png(fig: go.Figure, title: str, active_filters: dict) -> bytes:
    """
    Exports a chart as PNG with metadata footer.
    Returns PNG bytes for st.download_button().
    """
    # Add metadata footer to figure before export
    footer_text = (
        f"BUIP · Traffic Intelligence  |  "
        f"Filters: {format_filter_summary(active_filters)}  |  "
    ...
    # scale=2: 2400×1400px output — retina quality for print and presentation use
```

**Export filename convention:** `BUIP_[DashboardCode]_[ChartCode]_[YYYYMMDD]_[HHMMSS].png`  
Example: `BUIP_TRAFFIC_T05_20240815_143201.png`

---

## 7.3 · PDF Report Export

The platform supports a structured PDF export mode that compiles a full-page analytical summary. This is accessible via a persistent `⬇ Export Report` button in the filter strip (right-aligned, always visible).

**PDF report structure:**

```
Page 1: Cover
  - Platform name + dashboard name
  - Active filter summary (full specification)
  - Date generated + analyst name (if session includes user identification)
  - Dataset period covered

Page 2: Executive Summary
  - All KPI values for current filter state
  - One-sentence interpretation per KPI
  - Overall severity summary badge

Pages 3–N: Chart Pages (one chart per page)
  - Chart at full-page width
  - Chart title
  - Active annotation set (all annotations that were visible at time of export)
  - Insight panel text (if insight panel was expanded at time of export)
  - Filter state footer

Final Page: Data Notes
  - Dataset sources and versions
  - Methodology notes (rolling average windows, severity thresholds)
  - NAAQS and WHO standard references (for AQI exports)
  - "This report was generated from observational historical data..."
```

**PDF implementation approach:**

Use `reportlab` (server-side) or `weasyprint` (HTML → PDF) depending on the deployment environment. Charts are exported as PNG via `fig.to_image()` and embedded in the PDF. KPI tables are rendered as HTML tables via `weasyprint`.

```python
# PDF generation entry point in utils/export.py:
def generate_pdf_report(
    dashboard: str,          # "traffic" | "aqi"
    active_filters: dict,
...
## 7.4 · Executive Summary Export Mode

A stripped-down, one-page export for non-technical stakeholders (city officials, department heads):

**Executive summary content:**

- Platform: name, date range, filter summary
- Three headline findings — one sentence each, written analytically (not chart titles)
- Three KPI values with their severity assessment
- One hero chart (selected by export configuration or by the analyst at export time)

**Visual treatment for executive export:** White background, black text. The dark theme platform exports to light-background PDF for print readability. Chart backgrounds are set to `paper_bgcolor="white"` and `plot_bgcolor="#F8F9FA"` for the export render pass only.

---

## 7.5 · Filter-Preserving and Annotation-Preserving Exports

**Filter state preservation:** Every export renders the data as it appears under the current filter state. The filter state is embedded in both the visual footer and the PDF metadata. An analyst who receives an export and wants to reproduce it in the platform can read the filter specification from the footer and recreate it exactly.

**Annotation preservation:** All visible annotations (quadrant labels, reference lines, callout boxes, SHAP values if open) are present in exports. The `fig.to_image()` render captures the full Plotly figure state including annotations. Streamlit-level annotations (inside `st.markdown()` below the chart) are captured via the insight panel text that travels to the PDF report.

---

## 7.6 · Dark Mode Export Handling

Charts exported from the dark-theme platform require special handling to remain legible in light-context documents (PDFs, presentations, emails):

**Export color remapping:**

```python
EXPORT_LAYOUT_OVERRIDES = {
    "paper_bgcolor": "white",
    "plot_bgcolor": "#F0F0F0",
    "font": {"color": "#1A1A1A"},
    "xaxis": {"gridcolor": "#D0D0D0", "linecolor": "#666666"},
    "yaxis": {"gridcolor": "#D0D0D0", "linecolor": "#666666"},
    "hoverlabel": {"bgcolor": "white", "font": {"color": "#1A1A1A"}},
}

def apply_export_theme(fig: go.Figure) -> go.Figure:
    """Apply light-theme overrides for print/PDF export."""
    fig.update_layout(**EXPORT_LAYOUT_OVERRIDES)
    # Data trace colors: already specified as explicit hex, do not remap
    # Exception: TRAFFIC_CRIMSON is already high-contrast on white — preserved
    return fig
```

**Severity color preservation in exports:** TRAFFIC_CRIMSON (#E5383B) and TRAFFIC_AMBER (#FFBA08) remain unchanged in export mode — they are already legible on white backgrounds. TRAFFIC_TEAL (#2EC4B6) is lightened for export: `#1A9E96` (slightly darker) for better contrast on white.
